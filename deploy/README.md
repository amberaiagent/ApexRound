...
HTTPS and canonical redirects have been verified through Cloudflare. Certbot's simulated renewal, including the Nginx deploy hook, passed. The Cloudflare account setting **Full (strict)** still needs confirmation in the owner's dashboard. At handoff the local Windows DNS resolver still returned the prior loopback address, while the `.com` delegation and remote public resolvers returned the active Cloudflare setup. Do not replace DNS with a permanent hosts-file override; use the SSH preview while caches update.
