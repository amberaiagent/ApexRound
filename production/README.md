...
1. **Token:** obtain the owner's official $APEX contract address. Verify bytecode, network, decimals and token identity. `dist/lib/config.js` deliberately leaves `tokenAddress` and `decimals` null. The adapter checks bytecode, decimals and balance at one block and compares integers exactly. Its result is informational; an authoritative backend must repeat verification when accepting entries. No actual $APEX balance was tested because its address has not been supplied.
