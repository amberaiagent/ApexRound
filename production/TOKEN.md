...
4. This repeats the onchain checks, writes the verified token and one activation timestamp to the persistent server database, and **immediately starts the main 02:00:00 timer and opens registration**. The timestamp is set after successful verification, not when inspection begins.
